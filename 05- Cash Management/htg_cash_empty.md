# Terminal Cash-up and Empty
# Introduction

This document provides full description of the Cash Management & Empty
Process in the Enactor Retail System from the Operational perspective.
It describes its purpose, its foundations and the Cash Management
Operations by which it is performed and recorded.
...
POS to support Terminal Cash Up & Empty with opening float.
...
The following steps are required to enable terminal cash up and empty
with opening float:
...
-   **Menu** -- Adding cash up and empty, including opening float
    function

-   **Role** -- Ensuring **POS** users have the appropriate privileges
    to carry out terminal cash up and empty with opening float
    functionality

-   **Tenders** -- Enabling tenders for terminal cash up and empty with
    opening float
...
\*if you wish for the terminal cash up and empty function to move the
counted totals from the terminal cash session to the safe session, a
configured safe is required against the store location.

![](./cash_empty/media/image3.png)
...
If the Terminal Cash up And Empty function and the open float function
does not exist in the menu, highlight the Terminal Cash Management
option add the functions.

Select Add \> Add a new button.

![](./cash_empty/media/image8.png)

The Terminal Cash up & Empty Button should be configured as follows:

![](./cash_empty/media/image9.png)

Set the values on the **General** tab as follows:

| Configuration | Description                                                                                                                                 |
|---------------|---------------------------------------------------------------------------------------------------------------------------------------------|
| Event         | Select Cash up and Empty from the drop-down selection, some of the configurations in this list will pre-populate on selection of the event. |
| ID            | The ID should be a unique ID of your choice                                                                                                 |
| Button Label  | Configure a button label if you do not have an image URL configured.                                                                        |
| Image URL     | NextGen1024x768/Admin/cashupandempty.png                                                                                                    |
| Position      | Where you want the function to be displayed on the menu.                                                                                    |
The Open Float Button should be configured as follows:

![](./cash_empty/media/image10.png)

Set the values on the **General** tab as follows:

| Configuration         | Description                                                                                                                             |
|-----------------------|-----------------------------------------------------------------------------------------------------------------------------------------|
| Event                 | Select Opening Float from the drop-down selection, some of the configurations in this list will pre-populate on selection of the event. |
| ID                    | The ID should be a unique ID of your choice                                                                                             |
| Button Label          | Configure a button label if you do not have an image URL configured.                                                                    |
| Image URL             | NextGen1024x768/Admin/openingfloat.png                                                                                                  |
| Visibility Expression | #{priv:checkPriv ilege('enactor.terminalOpeningFloat.Run')}                                                                             |
|                       |                                                                                                                                         |
|                       | The function will be visible if the user has the enactor.terminalOpeningFloat.Run privilege assigned to their role.                     |
| Position              | Where you want the function to be displayed on the menu.                                                                                |
|                       |                                                                                                                                         |
...
![](./cash_empty/media/image13.tiff)

Edit the tenders that should be included in the terminal cash up and
empty process, the example captured is using CASH_UK tender.

![](./cash_empty/media/image14.png)
...
Select the opening float function on the POS, if prompted select the
float reason and press ok to continue, the operator has the option to
void the function if they wish to exit and void the process.

![](./cash_empty/media/image21.png)
...
![](./cash_empty/media/image22.png)

Enter by denomination functions:

| Function                | Description.                                                             |
|-------------------------|--------------------------------------------------------------------------|
| OK                      | Select Highlighted denomination                                          |
| Up                      | Moves to next line up                                                    |
| Down                    | Moves to next line down                                                  |
| Enter by Quantity/Value | Allows the user to enter the value by either quantity or value (default) |
| Back                    | Completes the function                                                   |



Select the Terminal Cash Up & Empty function on the POS, if prompted
select the reason and press ok to continue, the operator has the option
to void the function if they wish to exit and void the process.

![](./cash_empty/media/image23.png)
...
If any there are discrepancies over the configured tender terminal
threshold the POS will list the tenders with the discrepancy, no values
will be shown.

![](./cash_empty/media/image24.png)

Cash up Discrepancy Functions:

| Function | Outcome                         |
|----------|---------------------------------|
| OK       | Continue and accept discrepancy |
| Voids    | Void Event                      |
| Recount  | Recount tenders                 |
